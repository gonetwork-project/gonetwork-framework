...
test('test channel', function (t) {
  function setup (assert) {
    myState = new channelState.ChannelState({
      depositBalance: new util.BN(123),
      address: pkAddr[0].address
    })

    peerState = new channelState.ChannelState({
      depositBalance: new util.BN(200),
      address: pkAddr[1].address
    })
    ...
    testLocks = locks.map(function (lock) {
    ...
    assert.equals(peerState.address.compare(pkAddr[1].address), 0)
  ...
  t.test('channel component test: direct transfer create and handle', function (assert) {
    ...
    assert.throws(function () {
      directTransfer.from // eslint-disable-line
    }, 'no signature to recover address from caught correctly')

    directTransfer.sign(pkAddr[0].pk)
    // make sure direct transfer was created properly
    assertDirectTransfer(assert, directTransfer, pkAddr[0].address, 1, address, 10, Buffer.alloc(32), pkAddr[1].address)
    ...
    directTransfer.sign(pkAddr[0].pk)
    // make sure direct transfer was created properly
    assertDirectTransfer(assert, directTransfer, pkAddr[0].address, 2, address, 10 + 50, Buffer.alloc(32), pkAddr[1].address)
    ...
    transferrable = channel.transferrableFromTo(channel.myState, channel.peerState)
    ...
    assert.throws(function () {
      peerDirectTransfer.from // eslint-disable-line
    }, 'no signature to recover address from caught correctly')

    // peer sign
    peerDirectTransfer.sign(pkAddr[1].pk)
    // make sure direct transfer was created properly
    assertDirectTransfer(assert, peerDirectTransfer, pkAddr[1].address, 1, address, 250, Buffer.alloc(32), pkAddr[0].address)
  ...
  t.test('channel component test: direct transfer should not createDirectTransfer when transferredAmount > transferrable', function (assert) {
    ...
    assert.throws(function () {
      channel.createDirectTransfer(msgID, transferredAmount)
  ...
  t.test('channel component test: direct transfer should not handleTransfer when transferredAmount < state.transferredAmount', function (assert) {
    ...
    var directTransfer = createDirectTransfer(msgID, 1, transferredAmount, address, Buffer.alloc(32), pkAddr[1].address)
    directTransfer.sign(pkAddr[0].pk)
    // make sure direct transfer was created properly
    assertDirectTransfer(assert, directTransfer, pkAddr[0].address, 1, address, 40, Buffer.alloc(32), pkAddr[1].address)
    channel.handleTransfer(directTransfer, new util.BN(2))
    assertStateBN(assert, myState, 1, 123, 40, 0, 0)
    assertStateBN(assert, peerState, 0, 200, 0, 0, 0)

    var directTransferFail = createDirectTransfer(msgID, 2, transferredAmountFail, address, Buffer.alloc(32), pkAddr[1].address)
    directTransferFail.sign(pkAddr[0].pk)
    // make sure direct transfer was created properly
    assertDirectTransfer(assert, directTransferFail, pkAddr[0].address, 2, address, 30, Buffer.alloc(32), pkAddr[1].address)
  ...
  t.test('channel component test: direct transfer should not handleTransfer when transferredAmount > transferrable', function (assert) {
    ...
    directTransfer.sign(pkAddr[0].pk)
    // make sure direct transfer was created properly
    assertDirectTransfer(assert, directTransfer, pkAddr[0].address, 1, address, 124, Buffer.alloc(32), pkAddr[1].address)
  ...
  t.test('channel component test: direct transfer should not handleTransfer with invalid locksroot', function (assert) {
    ...
    // (msgID,nonce,transferredAmount,channelAddress,locksRoot,to)
    var directTransfer = createDirectTransfer(msgID, 1, transferredAmount, address, testLocks[0].getMessageHash(), pkAddr[1].address)
    directTransfer.sign(pkAddr[0].pk)
    // make sure direct transfer was created properly
    assertDirectTransfer(assert, directTransfer, pkAddr[0].address, 1, address, 120, testLocks[0].getMessageHash(), pkAddr[1].address)
  ...
  t.test('channel component test: direct transfer should not handleTransfer with decremented nonce', function (assert) {
    ...
    directTransfer.sign(pkAddr[0].pk)
    // make sure direct transfer was created properly
    assertDirectTransfer(assert, directTransfer, pkAddr[0].address, -1, address, 50, Buffer.alloc(32), pkAddr[1].address)
  ...
  t.test('channel component test: direct transfer should not handleTransfer with nonce > nonce+1', function (assert) {
    ...
    var transferredAmount = new util.BN(50)
    // (msgID,nonce,transferredAmount,channelAddress,locksRoot,to)
    var directTransfer = createDirectTransfer(msgID, 2, transferredAmount, address, Buffer.alloc(32), pkAddr[1].address)
    directTransfer.sign(pkAddr[0].pk)
    // make sure direct transfer was created properly
    assertDirectTransfer(assert, directTransfer, pkAddr[0].address, 2, address, 50, Buffer.alloc(32), pkAddr[1].address)
  ...
  t.test('channel component test: mediated transfer create and handle', function (assert) {
    ...
    assert.throws(function () {
      mediatedtransfer.from // eslint-disable-line
    }, 'no signature to recover address from caught correctly')

    mediatedtransfer.sign(pkAddr[0].pk)

    // make sure mediated transfer was created properly
    assertMediatedTransfer(
      assert, mediatedtransfer, pkAddr[0].address, 1, address, 0,
      testLocks[0].getMessageHash(), pkAddr[1].address, pkAddr[1].address, pkAddr[0].address)
    ...
    var secretReveal = createRevealSecret(pkAddr[0].address, locks[0].secret)
    ...
    secretToProof.sign(pkAddr[0].pk)
  ...
  t.test('channel component test: mediatedTransfer should not accept with different locksRoot ', function (assert) {
    ...
    invalidLocksRoot = createMediatedTransfer(1, 18, 50, address, invalidLocksRoot.getRoot(), address,
      address, address, testLocks[2])
    invalidLocksRoot.sign(pkAddr[0].pk)
  ...
  t.test('channel component test: mediatedTransfer should not accept with differing locksRoot; even if set to zero buffer ', function (assert) {
    ...
    invalidLocksRoot = createMediatedTransfer(1, 18, 50, address, Buffer.alloc(32), address,
      address, address, testLocks[2])
    invalidLocksRoot.sign(pkAddr[0].pk)
  ...
  t.test('channel component test: mediatedTransfer cannot register same lock twice; both in pending', function (assert) {
    ...
    var duplicateTransfer = channel.createMediatedTransfer(
      ...
      pkAddr[0].address,
      currentBlock)
    duplicateTransfer.sign(pkAddr[0].pk)
    try {
  ...
  t.test('channel component test: mediatedTransfer cannot register same lock twice; one in openLocks', function (assert) {
    ...
    // settleTimeout = 100;
    var testMT = computeMerkleTree(testLocks.slice(0, 2))
    // invalidLocksRoot = computeMerkleTree(testLocks.slice(1, 1))
  ...
  t.test('channel component test: mediatedTransfer should not accept with less transferredAmount ', function (assert) {
    ...
    invalidTransferredAmount.sign(pkAddr[0].pk)
  ...
  t.test('MANUAL TEST: channel can prove lock in solidity', function (assert) {
    ...
    var mediatedtransfer2 = channel.createMediatedTransfer(
      ...
      pkAddr[0].address,
      currentBlock)
    mediatedtransfer2.sign(pkAddr[0].pk)
    ...
    var mediatedtransfer3 = channel.createMediatedTransfer(
      ...
      pkAddr[0].address,
      currentBlock)
    mediatedtransfer3.sign(pkAddr[0].pk)
    ...
    var mediatedtransfer4 = channel.createMediatedTransfer(
      ...
      pkAddr[0].address,
      currentBlock)
    mediatedtransfer4.sign(pkAddr[0].pk)
    ...
    currentBlock = currentBlock.add(new util.BN(1))
    var secretReveal = createRevealSecret(pkAddr[0].address, locks[0].secret)
    var secretReveal2 = createRevealSecret(pkAddr[0].address, locks[1].secret)
    secretReveal.sign(pkAddr[0].pk)
    secretReveal.sign(pkAddr[0].pk)
  ...
  t.test('channel can only close once and correct lockProofs generated against solidity', function (assert) {
    ...
    var secretReveal2 = createRevealSecret(pkAddr[0].address, locks[1].secret)
    secretReveal.sign(pkAddr[0].pk)
    secretReveal2.sign(pkAddr[0].pk)
    ...
    assertLockProofString = function (openLockProof, root, stringABIEncodedLock, stringProofHexString, stringRoot) {
    ...
    assertProof(assert, proof, new util.BN(2), channel.channelAddress, new util.BN(0), testMT.getRoot(), pkAddr[0].address)
  ...
  t.test('channel can call handleClose again in case of blockChain error', function (assert) {
    ...
    assertProof(assert, proof1, new util.BN(1), channel.channelAddress, new util.BN(0), testMT.getRoot(), pkAddr[0].address)
    assertProof(assert, proof2, new util.BN(1), channel.channelAddress, new util.BN(0), testMT.getRoot(), pkAddr[0].address)
  ...
  t.test('channel can call transferUpdate after on chain close initiated by partner', function (assert) {
    ...
    proof1 = peerChannel.issueTransferUpdate(currentBlock)
    assert.equals(peerChannel.updatedProofBlock, null)
    assert.equals(peerChannel.issuedCloseBlock, null, 'can reissue close block after error')
  ...
  t.test('channel can transition through out-of-order blockChain close and settlement; settle can only be issued SETTLE_TIMEOUT even if handleSettle issued multipleTimes', function (assert) {
    ...
    channel.issueClose(currentBlock)
  ...
  t.test('channel component test: mediated transfer can handle when lockedAmount transferred > remaining transferrable', function (assert) {
    ...
    var mediatedtransfer = channel.createMediatedTransfer(
      ...
      testLocks[0].amount,
      testLocks[0].expiration, // currentBlock = 5
      pkAddr[1].address, // target
      pkAddr[0].address, // initiator
      currentBlock)
