...
class BlockchainService {
   ...
   */
    getLogs(address, fromBlock, toBlock, topicsArray) {
   ...
   * @param {function} decode - an optional decode function that is run on the result data
   */
    fetchSimple(method, params, decode) {
        let randomInt = this._getRandomInt();
        return node_fetch_1.default(this.providerUrl, {
            "method": 'POST',
            "headers": {
    ...
    getContractData(nettingChannelAddress, from) {
        return this.fetchAndDecodeCall(NettingChannelContractAbi.data, nettingChannelAddress.toString("hex"), from.toString('hex'), []);
    }
    /*** retreives the channelPartnerAddress, transferredAmount, and locksRoot you submitted in the proof*/
    getParticipantData(nettingChannelAddress, from) {
        return this.fetchAndDecodeCall(NettingChannelContractAbi.participantData, nettingChannelAddress.toString("hex"), from.toString('hex'), []);
    }
    getTokenBalance(tokenAddress, from, nodeAddress) {
        return this.fetchAndDecodeCall(HumanStandardTokenAbi.balanceOf, tokenAddress.toString('hex'), from.toString('hex'), [util.addHexPrefix(nodeAddress.toString('hex'))]);
    }
    getAddressAndBalance(channelAddress, from) {
        return this.fetchAndDecodeCall(NettingChannelContractAbi.addressAndBalance, channelAddress.toString("hex"), from.toString("hex"), []);
    }
    getNettingContractsByAddress(channelManagerAddress, from, nodeAddress) {
        return this.fetchAndDecodeCall(ChannelManagerContractAbi.nettingContractsByAddress, channelManagerAddress.toString("hex"), from.toString("hex"), [util.addHexPrefix(nodeAddress.toString("hex"))]);
    }
    getClosingAddress(nettingChannelAddress, from) {
        return this.fetchAndDecodeCall(NettingChannelContractAbi.closingAddress, nettingChannelAddress.toString("hex"), from.toString("hex"), []);
    }
    fetchAndDecodeCall(functionRef, to, from, data) {
        return node_fetch_1.default(this.providerUrl, {
            "method": 'POST',
            "headers": {
                "contentType": 'application/json'
            },
            "body": JSON.stringify(this.ethCall([this._createCall(functionRef, util.addHexPrefix(to), util.addHexPrefix(from), data)]))
        })
    ...
    fetchAndDecodeTx(functionRef, nonce, gasPrice, gasLimit, to, value, params) {
        return node_fetch_1.default(this.providerUrl, {
            "method": 'POST',
            "headers": {
                "contentType": 'application/json'
            ...
            "body": JSON.stringify(this.ethSendRawTransaction(this._create(functionRef, nonce, gasPrice, gasLimit, to, value, params)))
        })
            .then(res => {
            if (res.status !== 200)
    ...
    transferToken(nonce, gasPrice, tokenAddress, to, amount) {
        return this.fetchAndDecodeTx(HumanStandardTokenAbi.transfer, nonce, gasPrice, 220000, // compiled gas limit * 20%
        tokenAddress, null, [to, amount]);
    }
    approve(nonce, gasPrice, tokenAddress, spender, amount) {
        return this.fetchAndDecodeTx(HumanStandardTokenAbi.approve, nonce, gasPrice, 220000, // compiled gas limit * 20%
        tokenAddress, null, [spender, amount]);
    }
    newChannel(nonce, gasPrice, channelAddress, partnerAddress, timeout) {
        return this.fetchAndDecodeTx(ChannelManagerContractAbi.newChannel, nonce, gasPrice, 2200000, // compiled gas limit * 20%
        channelAddress, null, [partnerAddress, timeout]);
    }
    deposit(nonce, gasPrice, nettingChannelAddress, amount) {
        return this.fetchAndDecodeTx(NettingChannelContractAbi.deposit, nonce, gasPrice, 90000, // compiled gas limit * 20%
        nettingChannelAddress, null, [amount]);
    }
    close(nonce, gasPrice, nettingChannelAddress, proof) {
        return this.fetchAndDecodeTx(NettingChannelContractAbi.close, nonce, gasPrice, 2400000, // compiled gas limit * 20%
        nettingChannelAddress, null, [proof.nonce, proof.transferredAmount, proof.locksRoot, proof.messageHash, this.solidityPackSignature(proof.signature)]);
    }
    updateTransfer(nonce, gasPrice, nettingChannelAddress, proof) {
        return this.fetchAndDecodeTx(NettingChannelContractAbi.updateTransfer, nonce, gasPrice, 2400000, // compiled gas limit * 20%
        nettingChannelAddress, null, [proof.nonce, proof.transferredAmount, proof.locksRoot, proof.messageHash, this.solidityPackSignature(proof.signature)]);
    }
    withdrawLock(nonce, gasPrice, nettingChannelAddress, encodedLock, merkleProof, secret) {
        ...
        return this.fetchAndDecodeTx(NettingChannelContractAbi.withdraw, nonce, gasPrice, 2400000, // compiled gas limit * 20%
        nettingChannelAddress, null, [encodedLock, merkleProofBytes, secret]);
    }
    settle(nonce, gasPrice, nettingChannelAddress) {
        return this.fetchAndDecodeTx(NettingChannelContractAbi.settle, nonce, gasPrice, 1200000, // compiled gas limit * 20%
        nettingChannelAddress, null, []);
    }
