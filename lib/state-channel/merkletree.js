...
function MerkleTree(elements, ordered) {
    ...
    for (let i = 0; i < this.elements.length; i++) {
        ...
        let buffer = util.toBuffer(this.elements[i]);
        if (!(buffer.length === 32)) {
...
MerkleTree.prototype._getProofPair = function (index, level) {
    let pairIndex = (index + 1) % 2 === 0 ? index - 1 : index + 1;
...
MerkleTree.prototype.remove = function (hashedElement) {
    let index = this.findElement(hashedElement);
...
exports.checkMerkleProofOrdered = checkMerkleProofOrdered;
...
exports.checkMerkleProof = checkMerkleProof;
