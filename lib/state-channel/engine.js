...
class Engine extends events.EventEmitter {
            ...
            if (event.startsWith('GOT.')) {
                ...
                switch (event) {
                    case 'GOT.sendMediatedTransfer':
                        ...
                        let mediatedTransfer = channel.createMediatedTransfer(state.msgID, state.lock.hashLock, state.lock.amount, state.lock.expiration, state.target, state.initiator, state.currentBlock);
    ...
    onChannelNew(channelAddress, addressOne, addressTwo, settleTimeout) {
        ...
        let channel = new channelLib.Channel(stateTwo, stateOne, channelAddress, this.currentBlock);
