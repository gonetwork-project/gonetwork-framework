...
// be aware that some tests are env dependent and may overwrite this setting
const cmdEnv: Env = process.argv[process.argv.length - 1] as Env || 'infura'

if (!(['infura', 'local'] as Env[]).find(e => e === cmdEnv)) throw new Error(`UNSUPPORTED_ENV ${cmdEnv}`)
...
export const config = <E extends Env>(env = cmdEnv) => {
  if (env === 'infura') {
    return infura
  }
